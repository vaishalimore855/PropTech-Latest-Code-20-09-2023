...
import { getKycInvestorUserid } from "../../../../Redux/Slice/InvestorSlice";
import { Button, Modal, Box, TextField } from '@mui/material';
import { ToastContainer, toast } from "react-toastify";

function ViewInvestorPersonalDetails() {
  const { id } = useParams();
  const dispatch = useDispatch();
  const Investor = useSelector((state) => state.investor.Investor);
  const token = localStorage.getItem("token");
  const adminId = localStorage.getItem("adminId");
  const InvestorKyc = useSelector((state) => state.investor.KycUserid);
  useEffect(() => {
    dispatch(getInvestorById(id));
    dispatch(getKycInvestorUserid(id, token));
  }, [id]);
     ...
     <ToastContainer />
      <div className="view-document-main col-lg-12" style={{padding:"3%"}}>
        ...
        <dl className="row ">
          ...
          <dd className="col-7">{Investor?.personaldata?.firstname}</dd>
        </dl>
        <dl className="row ">
          <dt className="col-5">Last Name:</dt>
          <dd className="col-7">{Investor?.personaldata?.lastname}</dd>
        </dl>
        <dl className="row ">
          <dt className="col-5">Contact Number:</dt>
          <dd className="col-7">{Investor?.personaldata?.phoneNumber}</dd>
        </dl>
        <dl className="row ">
          <dt className="col-5">Email ID:</dt>
          <dd className="col-7">{Investor?.personaldata?.email}</dd>
        </dl>
        <dl className="row ">
          <dt className="col-5">User Role</dt>
          <dd className="col-7">{Investor?.personaldata?.userrole}</dd>
        </dl>
        ...
        <dl className="row ">
          ...
          <dd className="col-7">
          {
            Investor?.personaldata?.isVerifiedByAdmin == 'Verified'?
            <span class="badge badge-success">Verified</span>
            :
            Investor?.personaldata?.isVerifiedByAdmin == "Reject"?
        ...
        </dl>
        {
          ...
          <dd className="col-7">{Investor?.personaldata?.rejectReason}</dd>
          </dl>
          :
          null
        }
        ...
        <dl className="row">
          <dt className="col-5">KYC Document :</dt>
            ...
            <dl className="row">
            <dd className="col-4">
              {InvestorKyc?.aadhar_front_base64 ? (
                <div
                  style={{
                ...
                >
                  <img
                    src={`${InvestorKyc?.aadhar_front_base64}`}
                    alt=""
                    width="100%"
                    height="100%"
                  />
            ...
            <dd className="col-4">
              {InvestorKyc?.aadhar_back_base64 ? (
                <div
                  style={{
                    height: "170px",
                    border: "1px black solid",
                ...
                >
                  <img
                    src={`${InvestorKyc?.aadhar_back_base64}`}
                    alt=""
                    width="100%"
                    height="100%"
                  />
...
export default ViewInvestorPersonalDetails;
